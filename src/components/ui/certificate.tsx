            ...
            {certificates.length > 0 ? (
                <div className="w-full space-y-2 divide-y divide-gray-300">
                    {certificates.map((cert) => (
                        <div key={cert.id} className="min-h-18 space-y-1 flex">
                            <img src="/pdf.svg" alt="Logo" className="h-[40px] w-[40px]" />
                            <div>
                                <Typography variant="h5" className="text-xs text-rencanakan-type-black"> {cert.file.name} </Typography>
                                <Typography variant="p3">{formatFileSize(cert.file.size)}</Typography>
                            </div>
                        </div>
                    ))}
                </div>
